...
def _spectral_seed_labels(G, k, seed=0):
    ...
    computes the leading eigenvectors of the sparse normalized adjacency and is
    usable on the same sparse graphs as the SE optimizer itself.  The returned
    partition is only an initialization: final restart selection still uses the
    exact 2D structural-entropy objective.
    """
    import numpy as _np
    ...
    if k >= n - 1 or n <= max(32, 2 * k + 1):
        ...
        # Largest normalized-adjacency eigenvectors equal the smallest
        # normalized-Laplacian eigenvectors.  A seeded v0 makes ARPACK's start
        # reproducible without materializing a dense matrix.
    ...
    for _ in range(25):
        ...
        for j in range(k):
            ...
            if m.any():
                C[j] = X[m].mean(0)
    # Ensure exactly k non-empty clusters without emptying a different cluster.
    for j in range(k):
        if not (lab == j).any():
