...
\begin{tikzpicture}[
  ...
  internal/.style={circle,draw,inner sep=1.4pt},
  module/.style={rounded corners,draw,minimum width=8mm,minimum height=5mm},
  edge from parent/.style={draw,-}
]
\node[internal] (p1) at (0,0) {$P$};
\node[internal] (x) at (-0.7,-0.8) {$X$};
\node[module] (c1) at (0.7,-0.8) {$C$};
\node[module] (a1) at (-1.1,-1.65) {$A$};
\node[module] (b1) at (-0.3,-1.65) {$B$};
\draw (p1)--(x) (p1)--(c1) (x)--(a1) (x)--(b1);
\node at (2.0,-0.85) {$\xrightarrow{\mathrm{NNI}}$};
\node[internal] (p2) at (4,0) {$P$};
\node[module] (a2) at (3.3,-0.8) {$A$};
\node[internal] (y) at (4.7,-0.8) {$Y$};
\node[module] (b2) at (4.3,-1.65) {$B$};
\node[module] (c2) at (5.1,-1.65) {$C$};
\draw (p2)--(a2) (p2)--(y) (y)--(b2) (y)--(c2);
\end{tikzpicture}
\caption{A rooted NNI changes one internal module from $A\cup B$ to
$B\cup C$ while preserving all leaves and the surrounding tree.}
\label{fig:nni}
\end{figure}

\begin{theorem}[Exact weighted NNI delta]\label{thm:delta}
For $((A,B),C)\rightarrow(A,(B,C))$, the change in tree structural entropy is
\begin{equation}
 \Delta \HT=
 \frac{2}{\vol(V)}\left[
