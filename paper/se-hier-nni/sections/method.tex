\section{Entropy-Guided Tree Search}
Our framework contains two complementary optimizers. \EGTE searches a broad
space of non-binary hierarchy edits and provides a strong general solution.
\NEST uses the exact local identity developed below to obtain a substantially
faster hierarchy with an explicit NNI-local certificate.

\subsection{Entropy-guided general tree editing}
\EGTE first constructs a pool containing an SE-agglomerative dendrogram, a
recursive-SE partition tree, and a Paris tree when available. For each start it
searches two topology-changing neighborhoods. A \emph{collapse} removes an
internal level and connects its children directly to its parent. A
\emph{relocation} detaches a subtree and attaches it to a nearby internal
module, allowing information from one branch to reorganize another. Every
candidate tree is evaluated with the complete objective in
Eq.~\eqref{eq:tree-se}; a move is accepted only when this independently
recomputed value strictly decreases. Small graphs use best improvement over
all candidates, while larger graphs use shuffled first improvement with a
declared relocation cap.

\begin{proposition}[Monotonicity of general tree editing]\label{prop:egte}
The entropy returned by \textup{\EGTE} is no greater than that of the best successfully
constructed initial tree.
\end{proposition}
\begin{proof}
Each edit is committed only after exact recomputation verifies a strict
decrease, and \EGTE returns the lowest-entropy refined candidate.
\end{proof}

Collapse and relocation provide a broad edit space, but repeated full-tree
evaluation is costly and their termination does not characterize a canonical
binary neighborhood. We therefore develop an exact NNI refiner whose local
effect can be evaluated without rescoring every unaffected module.
...
entropy. Unlike \EGTE, it omits general collapse/relocation search and uses the
exact NNI neighborhood directly. The initializer is recorded for every output,
...
alternatives per sweep, versus the larger source--destination edit space of
\EGTE. Our implementation uses best improvement, cached leaf sets, sparse
